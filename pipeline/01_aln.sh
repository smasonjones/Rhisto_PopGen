#!/usr/bin/bash
#SBATCH --mem 64G --ntasks 12 --nodes 1 -J RhizAln
#SBATCH --out logs/Rhisto1.bwa.%a.log --time 1-00:00:00
...
IFS=,
cat $SAMPFILE | sed -n ${N}p | while read SAMPLE PAIR1 PAIR2
do
...
#	   continue
 #  fi
  ...
  SAMFILE=$TEMP/$STRAIN.unsrt.sam
  SRTED=$TEMP/${STRAIN}.srt.bam
  DDFILE=$TEMP/${STRAIN}.DD.bam
  REALIGN=$TEMP/${STRAIN}.realign.bam
  RECAL=$TEMP/${STRAIN}.recal.bam
 ...
 #     if [ ! -f $RECAL.grp ]; then
                ...
                -R $REFGENOME \
                -I $REALIGN \
                --knownSites recal.vcf \
                -o $RECAL.grp
  #      fi
   #      if [ ! -f $RECAL.post.grp ]; then
                 java -Xmx$MEM -jar $GATK \
                -T BaseRecalibrator \
                -R $REFGENOME \
                -I $REALIGN \
    ...
    #    fi
     #   if [ ! -f $BAMDIR/$SAMPLE.recal.bam ]; then
      ...
      samtools view -O $HTCFORMAT --threads $CPU \
          --reference $REFGENOME -o $FINALFILE $RECAL
...
#	  --reference $REFGENOME -o $FINALFILE $REALIGN
  #    samtools index $FINALFILE
#      if [ -f $FINALFILE ]; then
#	# rm -f $DDFILE $REALIGN $RECAL
#	 # rm -f $(echo $REALIGN | sed 's/bam$/bai/')
#	 # rm -f $(echo $RECAL | sed 's/bam$/bai/')
